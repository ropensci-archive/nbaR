...
#'           
#' @field consumes  list(character)
#'           
#' @field produces  list(character)
#'           
#' @field url  character
#'           
#'
...
RestService <- R6::R6Class(
  ...
  public = list(
    ...
    `endPoint`,
      ...
      if (!missing(`consumes`)) {
        stopifnot(is.list(`consumes`),
                  length(`consumes`) != 0)
        lapply(`consumes`,
               function(x) stopifnot(is.character(x)))
        ## omit names as they should not be part of JSON representation
        self[["consumes"]] <- unname(`consumes`)
      }
      if (!missing(`produces`)) {
        stopifnot(is.list(`produces`),
                  length(`produces`) != 0)
        lapply(`produces`,
               function(x) stopifnot(is.character(x)))
        ## omit names as they should not be part of JSON representation
        self[["produces"]] <- unname(`produces`)
      }
      if (!missing(`url`)) {
