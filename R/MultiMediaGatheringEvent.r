  ...
  public = list(
    ...
    `projectTitle`,
      ...
      if (!missing(`depthUnitOfMeasurement`)) {
        stopifnot(is.character(`depthUnitOfMeasurement`),
      ...
      if (!missing(`bioStratigraphic`)) {
        stopifnot(is.list(`bioStratigraphic`),
                  length(`bioStratigraphic`) != 0)
        lapply(`bioStratigraphic`,
               function(x) stopifnot(R6::is.R6(x)))
        ## omit names as they should not be part of JSON representation
    ...
    toList = function() {
      MultiMediaGatheringEventList <- list()
        ...
        if (!is.null(self[["dateText"]])) {
            MultiMediaGatheringEventList[["dateText"]] <-
                  self[["dateText"]]
      }
        ...
        if (!is.null(self[["behavior"]])) {
            MultiMediaGatheringEventList[["behavior"]] <-
                  self[["behavior"]]
      }
        ...
        if (!is.null(self[["code"]])) {
            MultiMediaGatheringEventList[["code"]] <-
                  self[["code"]]
      }
        if (!is.null(self[["establishmentMeans"]])) {
            MultiMediaGatheringEventList[["establishmentMeans"]] <-
