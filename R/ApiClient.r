...
ApiClient  <- R6::R6Class(
  ...
  public = list(
    ...
    processResponse = function(response, returnObject) {

      if (httr::status_code(response) >= 200 && httr::status_code(response) <= 299) {
          # response ok
          # check if we get back list or single object
          contentStr <- httr::content(response, type='text', encoding='UTF-8')
          if  (substr(contentStr, 1, 1) == '[' & substr(contentStr, nchar(contentStr), nchar(contentStr)) == ']') {
