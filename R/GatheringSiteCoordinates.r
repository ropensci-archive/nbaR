...
#'           
#' @field coordinateErrorDistanceInMeters  integer
#'           
#' @field spatialDatum  character
#'           
...
GatheringSiteCoordinates <- R6::R6Class(
  ...
  public = list(
    ...
    `gridCellCode` = NULL,
    `gridQualifier` = NULL,
    `coordinateErrorDistanceInMeters` = NULL,
    `spatialDatum` = NULL,
    `geoShape` = NULL,
    initialize = function(
    `longitudeDecimal`,
    ...
    toList = function() {
      GatheringSiteCoordinatesList <- list()
        ...
        if (!is.null(self[["coordinateErrorDistanceInMeters"]])) {
            GatheringSiteCoordinatesList[["coordinateErrorDistanceInMeters"]] <-
                  self[["coordinateErrorDistanceInMeters"]]
      }
        if (!is.null(self[["spatialDatum"]])) {
            GatheringSiteCoordinatesList[["spatialDatum"]] <-
                  self[["spatialDatum"]]
      }
    ...
    print = function(...) {
       ...
       cat("Fields:\n")
           ...
           if (typeof(self$coordinateErrorDistanceInMeters) == "environment") {
               cat("\tcoordinateErrorDistanceInMeters:\tobject of class", paste0("<", class(self$coordinateErrorDistanceInMeters)[1], ">"), "\n")
           }
           else if (typeof(self$coordinateErrorDistanceInMeters) == "list") {
               cat("\tcoordinateErrorDistanceInMeters:\tlist of length", length(self$coordinateErrorDistanceInMeters), "\n")
           }
           else {
               cat("\tcoordinateErrorDistanceInMeters:\t", self$coordinateErrorDistanceInMeters, "\n")
           }           
           if (typeof(self$spatialDatum) == "environment") {
               cat("\tspatialDatum:\tobject of class", paste0("<", class(self$spatialDatum)[1], ">"), "\n")
           }
           else if (typeof(self$spatialDatum) == "list") {
               cat("\tspatialDatum:\tlist of length", length(self$spatialDatum), "\n")
           }
           else {
               cat("\tspatialDatum:\t", self$spatialDatum, "\n")
           }           
