...
young_name <- sapply(res$content$resultSet, function(x)x$item$gatheringEvent$chronoStratigraphy[[1]]$youngChronoName)
old_name <- sapply(res$content$resultSet, function(x)x$item$gatheringEvent$chronoStratigraphy[[1]]$oldChronoName)

## translate this into years; get min young_name, max old_name
lower_bounds <- sapply(young_name, function(x)(min(unlist(get_geo_age_range(x)))))
upper_bounds <- sapply(old_name, function(x)(max(unlist(get_geo_age_range(x)))))
