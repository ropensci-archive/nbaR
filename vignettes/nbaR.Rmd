...
* **exists**: check if a record exists, based on its *unitID*:
```{r}
...
* **find**: return a single specimen given its identifier (Note: the identifier of a specimen is different 
...
* **findByIds**: Same as `find`, but takes multiple IDs:
```{r}
ids <- "RMNH.INS.657083@CRS,L.1589244@BRAHMS"
res <- sc$find_by_ids(ids)
```

* **findByUnitID**: Find a specimen by its `unitID`:
```{r}
