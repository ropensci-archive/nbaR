...
filename <- tempfile(fileext=".zip")
qs <- QuerySpec$new(conditions=list(QueryCondition$new(field="identifications.defaultClassification.genus", operator="EQUALS", value="Hydrochoerus")))
...
tc <- TaxonClient$new()
qc <- QueryCondition$new(field="acceptedName.genusOrMonomial", operator="EQUALS", value="Sedum")
...
```{r}
# get counts for taxa of genus 'Sedum' that are in the Netherlands Soortenregister (NSR)
qc <- QueryCondition$new(field="acceptedName.genusOrMonomial", operator="EQUALS", value="Sedum")
qc2 <- QueryCondition$new(field="sourceSystem.code", operator="EQUALS", value="NSR")
qs <- QuerySpec$new(conditions=list(qc, qc2))
tc$count(qs)
...
#### get_distinct_values
This method takes a specific field as an argument and returns all possible values and the frequency for that 
field in thy data. Example: get all data source systems for taxon objects: 
...
filename <- tempfile(fileext=".zip")
qs <- QuerySpec$new(conditions=list(QueryCondition$new(field="defaultClassification.genus", operator="EQUALS", value="Clematis")))
...
qc <- QueryCondition$new(field="identifications.scientificName.fullScientificName", operator="STARTS_WITH", value="Qu")
qs <- QuerySpec$new(conditions=list(qc), size=100)
res <- mc$query(qs)

# check if scientific names indeed start with 'Qu'
sapply(res$content$resultSet, function(x)x$item$identifications[[1]]$scientificName$fullScientificName)
```
...
##qc <- QueryCondition$new(field="gatheringEvent.chronoStratigraphy.chronoIdentifier", operator="NOT_EQUALS", value="")
##qs <- QuerySpec$new(conditions=list(qc))
##res <- sc$query(querySpec=qs)
##sc$get_distinct_values("identifications.defaultClassification.family", querySpec = qs)$content

##lapply(young_name, get_geo_age_range)
...
qc$or <- list(qc2)
qc3 <- QueryCondition$new(field="identifications.defaultClassification.family", operator="EQUALS", value="Carcharhinidae")

qs <- QuerySpec$new(conditions=list(qc, qc3), size=1000)
res <- sc$query(querySpec = qs)
...
data$early_age <- sapply(data$young_p, function(x)ifelse(is.na(x), NA, unlist(times['early_age', x])))
data$late_age <- sapply(data$old_p, function(x)ifelse(is.na(x), NA, unlist(times['late_age', x])))

require(ape)
tree <- read.nexus('other/sharks.nex')

spp <- paste(data$genus, data$species, sep="_")

## check which one of our data is in our tree
idx <- which(spp %in% tree$tip.label)
...
## translate this into years; get min young_name, max old_name
lower_bounds <- sapply(young_periods, function(x)(min(unlist(get_geo_age_range(x)))))



upper_bounds <- sapply(old_periods, function(x)(max(unlist(get_geo_age_range(x)))))

## get all genera
genera <- unlist(sapply(res$content$resultSet,
                      function(x)paste(x$item$identifications[[1]]$defaultClassification$genus)))

## put names to genera
names(lower_bounds) <- genera
names(upper_bounds) <- genera

upper_bounds_genera <- sapply(unique(genera), function(x)mean(upper_bounds[which(names(upper_bounds)==x)], na.rm=T))
lower_bounds_genera <- sapply(unique(genera), function(x)mean(lower_bounds[which(names(lower_bounds)==x)], na.rm=T))


lapply(unique(genera), function(x){
    grep (paste0("^", x, "_"), tree$tip.label)
})

    
## get all taxon names from NBA data
#specs <- unlist(sapply(res$content$resultSet,
#                      function(x)paste(x$item$identifications[[1]]$defaultClassification$genus,
#                                       x$item$identifications[[1]]$defaultClassification$specificEpithet, sep="_")))



## get lower and upper bounds for genera


##genera <- unlist(unique(lapply(res$content$resultSet, function(x)x$item$identifications[[1]]$defaultClassification$genus))

                 
##scoli <- res$content$resultSet[which(lapply(res$content$resultSet, function(x)x$item$identifications[[1]]$defaultClassification$genus) == "Scoliodon")]
##young_name <- sapply(scoli, function(x)x$item$gatheringEvent$chronoStratigraphy[[1]]$youngChronoName)
##old_name <- sapply(scoli, function(x)x$item$gatheringEvent$chronoStratigraphy[[1]]$oldChronoName)


##qc <- QueryCondition$new(field="gatheringEvent.biostratigraphy.bioDatingQualifier", operator="NOT_EQUALS", value="")
##qs <- QuerySpec$new(conditions=list(qc))
##res <- sc$query(querySpec=qs)
...
```{r, eval=FALSE}
## get the en tibi tomato specimen
...
sp$associatedMultiMediaUris[[1]]$accessUri
